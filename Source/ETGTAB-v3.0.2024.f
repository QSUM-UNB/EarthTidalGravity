...
C                                                                      C
C     Program ETGTAB, version 3.0 240810 FORTRAN 77.                   C
C                                                                      C
...
C     Last Modification by H.-G. Wenzel: 930821                        C
C     Last Modification by B. Barrett:   240810                        C
C       - Updated tables in ETMUTC to include leap seconds until 2025  C
C       - Floats in several FORMAT statements changed from F10.5 to    C
C         ES12.5 to properly output horizontal displacements (IC=3)    C
C**********************************************************************C
...
C      WRITE(IUN6,*) IW,DTHAM(IW),DTHPH(IW)
  ...
  910 CONTINUE
      CALL GEOEXT(IUN6,DEXTIM)
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C     Print hourly model tides with format 6(1X,ES12.5):               C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
 ...
 7013 FORMAT(///' Wave groups and observed tidal parameters'//
     ...
     2'                              [deg]'/)
 7014 FORMAT(2I5,1X,A8,ES12.4,F10.4)
 7015 FORMAT(3I5,2F10.4,1X,A8,ES12.4,F10.4)
 7016 FORMAT(////' Program ETGTAB, version ',A8,' FORTRAN 77'//
     1' Component ',A24,' IN ',1X,A8//4X,'Date in UT'//)
 7017 FORMAT(1X,I4,2(1X,I2.2),3(1X,I2.2),6(1X,ES12.5))
 7018 FORMAT(' Model tides from program ETGTAB, version',A10,' F77.'/
     ...
     *'INITIAL EPOCH (YYYY,MM,DD,HH)           ',5X,4I5/
     1'NUMERICAL LOW PASS FILTER (0...5)                2'/
     2'PRINT OBSERVATIONS (1=YES)                       0'/
     3'PRINT LOW-PASS FILTERED OBSERV. (1=YES)          0'/
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
  ...
  710 DPK(I)=DPK(I)+DATAN2(DGY(I),DGX(I))*DRO
      GOTO 2000
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C     IC=6, compute geodetic coefficients for aereal strain            C
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
 ...
 7003 FORMAT(//' Geodetic coefficients and phases for component',I4/
     ...
     2' GC 2,0',ES14.5,2X,A8,2X,F14.6,' deg'/
     3' GC 2,1',ES14.5,2X,A8,2X,F14.6,' deg'/
     4' GC 2,2',ES14.5,2X,A8,2X,F14.6,' deg'/
     5' GC 3,0',ES14.5,2X,A8,2X,F14.6,' deg'/
     6' GC 3,1',ES14.5,2X,A8,2X,F14.6,' deg'/
     7' GC 3,2',ES14.5,2X,A8,2X,F14.6,' deg'/
     8' GC 3,3',ES14.5,2X,A8,2X,F14.6,' deg'/
     9' GC 4,0',ES14.5,2X,A8,2X,F14.6,' deg'/
     *' GC 4,1',ES14.5,2X,A8,2X,F14.6,' deg'/
     1' GC 4,2',ES14.5,2X,A8,2X,F14.6,' deg'/
     2' GC 4,3',ES14.5,2X,A8,2X,F14.6,' deg'/
     3' GC 4,4',ES14.5,2X,A8,2X,F14.6,' deg'//)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C                                                                      C
C     Routine ETMUTC, version 180219 FORTRAN 77.                       C
C                                                                      C
C     All variables with D as first character are DOUBLE PRECISION.    C
C                                                                      C
C     Input parameter description:                                     C
...
C                  from 1955.5 until 2017.0 . For epochs less 1955.5,  C
C                  DDT is set to 31.59 s. For epochs exceeding 2017.0, C
C                  DDT is set to 69.184 s. ET is Ephemeris Time.       C
C                  TDT is Terrestrial Dynamical Time.                  C
C                  (since 2001 TDT replaced with TT = Terrestrial Time)C
C                  UTC is Universal Time Coordinated, as broadcasted byC
C                  radio or GPS satellites.                            C
C                                                                      C
C     The table DTAB should be extended when new data are available.   C
...
C                                                                      C
C     Last modification: 240810 B. Barrett                             C
C       - Updated table DTAB to include leap years until 2025          C
C**********************************************************************C
      IMPLICIT DOUBLE PRECISION (D)
      IMPLICIT INTEGER*4 (I-N)
      DOUBLE PRECISION DTAB(3,100)
...
C     - Update 240810, see data on websites:                           C
C       https://www.iers.org/IERS/EN/DataProducts/tools/timescales/timescales.html
C       https://en.wikipedia.org/wiki/Leap_second                      C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
     ...
     *            1985.50000D0,   2446247.50000D0,   55.184D0,
     ...
     1            1994.49999D0,   2449534.49999D0,   60.184D0,
     2            1994.50000D0,   2449534.50000D0,   61.184D0,
     3            1995.99999D0,   2450083.49999D0,   61.184D0,
     4            1996.00000D0,   2450083.50000D0,   62.184D0,
     5            1997.49999D0,   2450630.49999D0,   62.184D0,
     6            1997.50000D0,   2450630.50000D0,   63.184D0,
     7            1998.99999D0,   2451179.49999D0,   63.184D0,
     8            1999.00000D0,   2451179.50000D0,   64.184D0,
     9            2005.99999D0,   2453736.49999D0,   64.184D0,
     *            2006.00000D0,   2453736.50000D0,   65.184D0,
     1            2008.99999D0,   2454832.49999D0,   65.184D0,
     2            2009.00000D0,   2454832.50000D0,   66.184D0,
     3            2012.49999D0,   2456109.49999D0,   66.184D0,
     4            2012.50000D0,   2456109.50000D0,   67.184D0,
     5            2015.49999D0,   2457204.49999D0,   67.184D0,
     6            2015.50000D0,   2457204.50000D0,   68.184D0,
     7            2016.99999D0,   2457754.49999D0,   68.184D0,
     8            2017.00000D0,   2457754.50000D0,   69.184D0,
     9            2025.00000D0,   2460676.50000D0,   69.184D0/
      DATA NTAB/100/,IWARN/1/,ITAB/1/,N/1/
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
C     Format statements:                                               C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
 7001 FORMAT(//' Routine ETMUTC, version 240810 FTN 77.'//
     1' List of tables:'//
     2'       No.           Juld            DTX       DTY'//)
 7002 FORMAT(I10,2F15.5,F10.3)
 7051 FORMAT(/' ***** Warning from routine ETMUTC, version 240910.'/
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
 ...
 7011 FORMAT(ES12.5,1X,2F10.5,2X,A6,2X,F10.6)
 7028 FORMAT(8A10)
 7029 FORMAT(1X,8A10)
